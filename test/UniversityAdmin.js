...
contract('UniversityAdmin', (accounts) => {
  ...
  it('Should register the admin number', async () => {
    ...
    await contract.newAdmin(accounts[2], { from: accounts[0] });
    assert.equal(await contract.getAdminNumber(), 2);
    await contract.newAdmin(accounts[3], { from: accounts[0] });
    assert.equal(await contract.getAdminNumber(), 3);
    await contract.newAdmin(accounts[4], { from: accounts[0] });
  ...
  it('Only founder can add new admin', async () => {
    try {
      await contract.newAdmin(accounts[1], { from: accounts[1] });
    } catch (e) {
      return true;
    }
    throw new Error('Test failed!');
