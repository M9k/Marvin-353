...
contract('Student', (accounts) => {
  ...
  beforeEach('Deploy Student contract on blockchain', async () => {
    ...
    exam1 = await Exam.at(await course.getExamContractAt.call(0));
    await course.addNewExam(456, 10, false, { from: accounts[1] });
    exam2 = await Exam.at(await course.getExamContractAt.call(1));
...
/* TODO: NOT THERE!
  it('The teacher should register a valuation', async () => {
    await student.registerValuation(exam1.address, 19, { from: teacher.address });
    assert.equal(await student.getExamValuationAt.call(0), 19);
  });
  it('Shouldn\'t register incorrect valuation (negative)', async () => {
    try {
      await student.registerValuation(exam1.address, -1, { from: accounts[2] });
    } catch (e) {
      return true;
    }
    throw new Error('Test failed!');
  ...
  it('Shouldn\'t register incorrect valuation (too high)', async () => {
    try {
      await student.registerValuation(exam1.address, 33, { from: accounts[2] });
    } catch (e) {
      return true;
    }
    throw new Error('Test failed!');
  ...
  it('Shouldn\'t register if the student is unconfirmed', async () => {
    try {
